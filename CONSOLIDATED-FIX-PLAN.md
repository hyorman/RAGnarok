...
> - **Final-review 0.5.0 queue (2026-07-13)** — `getChunkId` adoption in `hybridRetriever`/`ensembleRetriever` (last ad-hoc keying sites); graph-path refine-attribution parity for `resultsCount`; a checkpoint-size regression test; align `engines` ranges on `">=20 <23"` (root/vscode currently `">=20"`); make the mcp-server README's ARCHITECTURE.md link npm-package-safe (GitHub URL); strengthen the dedup-in-average test with a two-score variant.
...
- [verified] Web ingestion SSRF guard: DNS resolution + private/loopback/link-local blocking (IPv4+IPv6) + **DNS-pinning** (defeats rebinding TOCTOU) + per-redirect revalidation + redirect cap. [todo/verify] GitHub host allowlist **behavior** (the `RAGNAROK_GITHUB_HOSTS` config exists and is documented; allowlist enforcement not yet exercised by a test); export/import traversal/zip-bomb protection (MB-9) — note the checksum half already has real E2E evidence: `stdioTransport.test.ts` asserts a tampered archive is rejected with "checksum mismatch".
