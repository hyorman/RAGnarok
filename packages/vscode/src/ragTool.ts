...
export class RAGTool {
  ...
  constructor(
    ...
    this.topicManager = Promise.resolve(topicManager);
  ...
  public async executeQuery(params: RAGQueryParams): Promise<RAGQueryResult> {
    try {
      ...
      logger.info(
        `Topic matched: ${topicMatch.topic.name} (${topicMatch.matchType}), ${stats.documentCount} documents, ${stats.chunkCount} chunks`,
      );
      ...
      const canUseWorkspaceContext =
        includeWorkspace && (await new QueryPlannerAgent(this.config, this.llmProvider).canRefineWithLLM(params.query));
      ...
      logger.info(
        `Query completed: ${formattedResults.results.length} results, confidence: ${ragResult.avgConfidence.toFixed(2)}`,
      );
  ...
  private async getOrCreateAgent(topicId: string): Promise<RAGAgent> {
    ...
    try {
      ...
      throw new Error(
        `Failed to load vector store for topic: ${topicId}. Error: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    if (!vectorStore) {
