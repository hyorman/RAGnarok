...
export class TopicTreeDataProvider implements vscode.TreeDataProvider<TopicTreeItem>, vscode.Disposable {
  private _onDidChangeTreeData: vscode.EventEmitter<TopicTreeItem | undefined | null | void> = new vscode.EventEmitter<
  ...
  constructor(topicManager: TopicManager, embeddingService: EmbeddingService) {
    ...
    this.modelChangeSubscription = { dispose: () => sub.unsubscribe() };
  }
  ...
  async getChildren(element?: TopicTreeItem): Promise<TopicTreeItem[]> {
    try {
      const topicManager = this.topicManager;
  ...
  private getStatisticsItems(stats: any): TopicTreeItem[] {
    ...
    // Document count
    items.push(new TopicTreeItem({ key: "document-count", value: stats.documentCount }, "stat-item"));

    // Chunk count
    items.push(new TopicTreeItem({ key: "chunk-count", value: stats.chunkCount }, "stat-item"));

    // Embedding model
    items.push(new TopicTreeItem({ key: "embedding-model", value: stats.embeddingModel }, "stat-item"));

    // Last updated
    const lastUpdated = new Date(stats.lastUpdated).toLocaleString();
    items.push(new TopicTreeItem({ key: "last-updated", value: lastUpdated }, "stat-item"));
...
export class TopicTreeItem extends vscode.TreeItem {
  constructor(
    ...
    super(TopicTreeItem.getLabel(data, type), TopicTreeItem.getCollapsibleState(type));
 ...
 */
export class ConfigTreeDataProvider implements vscode.TreeDataProvider<TopicTreeItem>, vscode.Disposable {
  private _onDidChangeTreeData: vscode.EventEmitter<TopicTreeItem | undefined | null | void> = new vscode.EventEmitter<
    TopicTreeItem | undefined | null | void
  >();
  readonly onDidChangeTreeData: vscode.Event<TopicTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;
  ...
  async selectVscodeEmbeddingModel(): Promise<void> {
    try {
      ...
      vscode.window.showInformationMessage(`VS Code embedding model set to: ${displayName}`);
  ...
  async selectLLMModel(): Promise<void> {
    try {
      ...
      pickerItems.sort((a, b) => {
        if (a.label === currentFamily) {
          return -1;
        }
        if (b.label === currentFamily) {
          return 1;
        }
      ...
      await config.update(CONFIG.LLM_MODEL, picked.label, vscode.ConfigurationTarget.Workspace);
        ...
        const input = await vscode.window.showInputBox({
          ...
          validateInput: (val) => {
            ...
            if (entry.min !== undefined && num < entry.min) {
              return `Minimum is ${entry.min}`;
            }
            if (entry.max !== undefined && num > entry.max) {
              return `Maximum is ${entry.max}`;
            }
            return undefined;
          },
        });
