...
export class SemanticChunker {
   ...
   */
  public async chunkDocuments(documents: LangChainDocument[], options: ChunkingOptions = {}): Promise<ChunkingResult> {
   ...
   */
  public async chunkDocument(document: LangChainDocument, options: ChunkingOptions = {}): Promise<LangChainDocument[]> {
  ...
  private determineStrategy(
    ...
    const hasMarkdown = sampleDocs.some((doc) => doc.metadata.isMarkdown || doc.metadata.fileType === "markdown");
   ...
   */
  private createSplitter(strategy: "markdown" | "recursive" | "code", options: ChunkingOptions) {
    ...
    switch (strategy) {
      ...
      case "recursive":
      default:
        return this.createRecursiveSplitter(chunkSize, chunkOverlap, options.customSeparators);
    }
  }
   ...
   */
  private createMarkdownSplitter(chunkSize: number, chunkOverlap: number): MarkdownTextSplitter {
   ...
   */
  private createCodeSplitter(chunkSize: number, chunkOverlap: number): RecursiveCharacterTextSplitter {
   ...
   */
  private enrichChunksInBatches(chunks: LangChainDocument[], options: ChunkingOptions): LangChainDocument[] {
   ...
   */
  private enrichSingleChunk(chunk: LangChainDocument, index: number, options: ChunkingOptions): LangChainDocument {
