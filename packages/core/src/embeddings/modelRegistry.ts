...
/**
 * Locate the assets/models directory by walking up from __dirname.
 * Works in both unbundled (packages/core/dist/embeddings/) and
 * bundled (dist/) directory layouts.
 */
function findAssetsModelsDir(): string | null {
  let dir = __dirname;
  for (let i = 0; i < 6; i++) {
    const candidate = path.join(dir, "assets", "models");
...
export class ModelRegistry {
  ...
  public getBundledModelsRoot(): string | null {
    ...
    const candidate = findAssetsModelsDir();
    ...
    if (candidate) {
      ...
      this.logger.debug("No bundled models directory found");
    }
   ...
   */
  public resolveLocalModelPath(configProvider?: IConfigProvider, workspaceFolderPath?: string): string | null {
    const configuredPath = (
      configProvider?.get<string>(CONFIG.LOCAL_MODEL_PATH, DEFAULTS.LOCAL_MODEL_PATH) ?? DEFAULTS.LOCAL_MODEL_PATH
    ).trim();
    if (!configuredPath) {
      return null;
    }
    ...
    const normalizedPath = path.isAbsolute(expandedPath)
      ? expandedPath
      : path.resolve(workspaceFolderPath ?? process.cwd(), expandedPath);

    if (!fs.existsSync(normalizedPath)) {
      throw new Error(
        `Local embedding model path "${configuredPath}" does not exist (resolved to "${normalizedPath}")`,
      );
    }
  ...
  public getResolvedLocalModelPath(): string | null {
    if (this.resolvedLocalModelPath) {
  ...
  private async listDownloadedRemoteModels(): Promise<string[]> {
    try {
      ...
      const normalizedCacheDir = path.isAbsolute(cacheDir) ? cacheDir : path.resolve(cacheDir);
  ...
  public async listAvailableModels(): Promise<AvailableModel[]> {
    ...
    for (const name of bundled) {
      if (ModelRegistry.CURATED_MODELS.includes(name)) {
        continue;
      }
