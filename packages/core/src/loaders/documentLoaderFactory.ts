...
export class DocumentLoaderFactory {
   ...
   */
  public async loadDocuments(filePathsOrOptions: (string | LoaderOptions)[]): Promise<LoadedDocument[]> {
    ...
    if (failed.length > 0) {
      ...
      const hasCriticalError = failed.some(
        (f) =>
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          if (recursive) {
            ...
            const subFiles = await this.collectFilesFromDirectory(fullPath, recursive, includeExtensions, depth + 1);
            files.push(...subFiles);
          }
