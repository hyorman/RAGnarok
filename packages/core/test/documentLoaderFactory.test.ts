...
describe("DocumentLoaderFactory", function () {
  ...
  describe("isSupported", function () {
    ...
    it("should return true for web URLs", function () {
      expect(DocumentLoaderFactory.isSupported("https://example.com/page")).to.be.true;
      expect(DocumentLoaderFactory.isSupported("http://docs.site.org/guide")).to.be.true;
    });
  });
  ...
  describe("isWebUrl", function () {
    ...
    it("should return true for https URLs", function () {
      expect(DocumentLoaderFactory.isWebUrl("https://example.com/page")).to.be.true;
    });
    ...
    it("should return false for non-http protocols", function () {
      expect(DocumentLoaderFactory.isWebUrl("ftp://files.example.com")).to.be.false;
  ...
  describe("loadDocument - text", function () {
    it("should load a text file", async function () {
      ...
      expect(result.documents[0].pageContent).to.include("Natural Language Processing");
  ...
  describe("loadDocument - markdown", function () {
    it("should load a markdown file", async function () {
      ...
      expect(result.documents[0].pageContent).to.include("Sample Markdown Document");
    });
  ...
  describe("loadDocument - html", function () {
    it("should load an HTML file and extract text", async function () {
      ...
      expect(result.documents[0].pageContent).to.include("Machine Learning Basics");
    ...
    it("should set html-specific metadata", async function () {
      ...
      expect(result.documents[0].metadata.title).to.equal("Sample HTML Document");
    });
  ...
  describe("loadDocuments", function () {
    it("should load multiple files at once", async function () {
      ...
      expect(results.map((r) => r.fileType)).to.include.members(["text", "markdown", "html"]);
    });
  ...
  describe("loadDocuments - directory expansion", function () {
    it("should expand a directory to its files", async function () {
      const results = await factory.loadDocuments([{ filePath: fixturesPath, recursiveDirectory: false }]);
    ...
    it("should return empty for directory with no supported files", async function () {
      ...
      const results = await factory.loadDocuments([{ filePath: unsupportedDir, recursiveDirectory: false }]);
    ...
    it("should load recursively from nested directories", async function () {
      ...
      const results = await factory.loadDocuments([{ filePath: nestedDir, recursiveDirectory: true }]);
    ...
    it("should not recurse when recursiveDirectory is false", async function () {
      const nestedDir = path.join(fixturesPath, "nested");
      const results = await factory.loadDocuments([{ filePath: nestedDir, recursiveDirectory: false }]);
  ...
  describe("loadDocument - html edge cases", function () {
    ...
    it("should extract title from malformed HTML", async function () {
      ...
      expect(result.documents[0].metadata.title).to.equal("Malformed HTML Test");
    });
